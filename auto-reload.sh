...
check_log_size() {
    ...
    if [ "$size" -ge "$max_bytes" ]; then
        ...
        echo "$(date '+%Y-%m-%d %H:%M:%S') - [日志轮转] $LOG_FILE 超过 ${LOG_MAX_SIZE_MB}MB，已轮转为 $LOG_FILE.1" >> "$LOG_FILE"
    fi
}
...
start_socks() {
    ...
    log "[SOCKS] 启动中..."
    /usr/sbin/danted -f /etc/danted.conf >> "$LOG_FILE" 2>&1 &
    CURRENT_SOCKS_PID=$!
...
monitor_socks() {
    ...
    local test_urls=("https://www.cloudflare.com" "https://www.google.com" "https://www.wikipedia.org")
    local url_index=0
    ...
    while true; do
        ...
        if [ "$curl_result" -eq 0 ]; then
            ...
            local curl_error
            curl_error=$(curl -v --socks5-hostname 127.0.0.1:1080 "$current_url" \
                --http1.1 \
                --tls-max 1.2 \
                2>&1 | grep -E 'Failed|error' || true)
